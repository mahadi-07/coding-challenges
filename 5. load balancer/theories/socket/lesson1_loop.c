 ...
 *
 * Difference from server.c: instead of accepting ONE client and exiting,
 * we loop forever. server_fd keeps listening; each client gets its own
 * socket (conn) which we handle and then close. Press Ctrl+C to stop.
 */
int main(void)
{
    int server_fd;
    ...
    // 1. create socket
    if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {   // note: < 0, the correct check
    ...
    printf("Server listening on port 8080 (Ctrl+C to stop)...\n");
